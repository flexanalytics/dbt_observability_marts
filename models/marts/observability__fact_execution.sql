...
with
    executions as (
        select
            ...
            as total_rowcount
        from
            {{ ref('dbt_observability_marts', 'observability__int_execution') }} e
        left outer join {{ ref('dbt_observability_marts', 'observability__stg_model') }} m on
            e.command_invocation_id = m.command_invocation_id
            and e.node_id = m.node_id
        left outer join {{ ref('dbt_observability_marts', 'observability__stg_source') }} s on
...
select
    ...
    status,
    case when lower(status) in ('pass', 'success') then 1 else 0 end as is_pass,
    case when lower(status) in ('fail', 'failure', 'error') then 1 else 0 end as is_failure,
    case when lower(status) = 'warn' then 1 else 0 end as is_warn,
    total_node_runtime,
    ...
    sum(total_node_runtime) over (partition by command_invocation_id) as total_invocation_runtime,
    total_rowcount,
