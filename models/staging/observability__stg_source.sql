...
{% set column_names = dbt_utils.get_filtered_columns_in_relation(from=ref(target_name, 'observability__sources')) %}
...
select
    ...
    {% endif %}
    total_rowcount
from {{ ref(target_name, 'observability__sources') }}
where exists (
    select *
    from {{ ref(target_name, 'observability__all_executions') }}
